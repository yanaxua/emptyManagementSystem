<template>
    <div class="calendar">
        <div class="calendar-tools">
            ...
            <span class="calendar-prev el-icon-d-arrow-left" @click="prevYear">
            ...
            <span class="calendar-next el-icon-d-arrow-right" @click="nextYear"></span>
        ...
        <table cellpadding="5" ref="baseCleandar">
            <thead>
            <tr>
            ...
            <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
                <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}"
                    ...
                    <label>
                        ...
                        <input type="text" v-if="editable" v-model.number="editInputs[k1][k2]['content']"
                               v-filterS="editInputs[k1][k2]['content']" :ref="'calendarInput'+k1+k2"
                               @click="indexK1=k1;indexK2=k2;" :disabled="child.disabled">
                    </label>
        ...
        <div class="calendar-years" :class="{'show':yearsShow}" ref="baseCalendarYears">
            <label v-for="y in years" @click="activeYear = y;">
                <!--@click.stop="selectYear(y)"-->
                <span :class="{'active':y==activeYear}">{{y}}</span>
                <label>
                    产量
                    <input type="text" v-if="editable" v-model.number="editYearInputs['harvest'][y]"
                           :readonly="readonly"
                           v-filterS="editYearInputs['harvest'][y]" :ref="'calendarYearInputharvest'+y"
                    >
                </label>
                <label>
                    能耗比例
                    <input type="text" v-if="editable" v-model.number="editYearInputs['energyScale'][y]"
                           :readonly="readonly"
                           v-filterS="editYearInputs['energyScale'][y]" :ref="'energyScale'+y"
                    >
                </label>

            </label>
        </div>
        <div class="calendar-months" :class="{'show':!yearsShow}">
            <label v-for="m in months" @click="activeMonth = m">
                <!--@click.stop="selectYear(m)"-->
                <span :class="{'active':m==activeMonth}">{{m}}</span>
                <label>
                    产量
                    <input type="text" v-if="editable" v-model.number="editMonthInputs['harvest'][m]"
                           v-filterS="editMonthInputs['harvest'][m]" :ref="'calendarMonthInputharvest'+year+m"
                    >
                </label>
                <label>
                    能耗比例
                    <input type="text"
                           v-if="editable"
                           v-model.number="editMonthInputs['energyScale'][m]"
                           v-filterS="editMonthInputs['energyScale'][m]"
                           :ref="'energyScale'+year+m"
                    >
                </label>
...
//                monthsShow: false,
        ...
        watch: {
            ...
            year(newV, oldV) {
                if (oldV != 0) {
                    this.$emit('changeYear', newV);//传出this.year
        ...
        methods: {
            ...
            init() {
                ...
                this.render(this.year, this.month)
                this.changeYear();
                this.activeMonth = this.months[this.month - 1]
            },
            // 渲染日期
            render(y, m) {
            ...
            //上年
            prevYear() {
                if (this.years.findIndex(item => item == this.year) == 0) {
                    this.changeYear();
                }
                this.year = parseInt(this.year) - 1;
            ...
            //下年
            nextYear() {
                if (this.years.findIndex(item => item == this.year) == 11) {
                    this.changeYear();
                }
                this.year = parseInt(this.year) + 1;
                this.activeYear = this.year;
...
//        console.log(this.rangeEnd, 'rangeEnd');
                ...
                if (this.range) {
                    if (this.rangeBegin.length == 0 || this.rangeEndTemp != 0) {
                        ...
                        this.$emit('select', begin, end, '12345')
                    }
                    this.render(this.year, this.month)
                } else if (this.multi) {
                    // 如果已经选过则过滤掉
                    ...
                    this.today = [k1, k2]
                    this.$emit('select', [this.year, this.zero ? this.zeroPad(this.month + 1) : this.month + 1, this.zero ? this.zeroPad(this.days[k1][k2].day) : this.days[k1][k2].day, this.editInputs[k1][k2]["content"]])
                }
            },
            changeYear() {
//                if (this.yearsShow) {
...
//                this.yearsShow = true
                this.years = [];
                for (let i = ~~this.year - 6; i < ~~this.year + 6; i++) {
                    this.years.push(i)
                }
            },
            selectYear(value) {
            ...
            //获取input的值
            getInputs() {
                let inputContent = {}, me = this, indexes = 0, start;
                //item每行
                this.days.forEach((item, index) => {
                    ...
                    item.forEach((value, key) => {
                        if (!value.disabled) {
                            ...
                            let time = me.year + "-" + me.month + "-" + value.day;
                            inputContent[time] = me.editInputs[index][key]["content"];
                        }
                    })
                })
                return inputContent
            ...
            // 设置input的值
            setInputs(data, label) {
                let me = this, indexes = 0, start;
                this.editInputs.forEach((item, index) => {
                    ...
                    item.forEach((value, key) => {
                        if (!value.disabled) {
                            ...
                            ;
                            if (data[index * 7 + key - start]) {
                                value["content"] = data[index * 7 + key - start][label]
                            }
                        }
                    })
                })
            ...
            //获取年的input的值
            getYearInputs() {
                return JSON.parse(JSON.stringify(this.editYearInputs))
            },
            //设置年的值
            setYearInputs(data) {
                //假设data为对应的时间点
                Array.from(data).forEach(item => {
                    this.$set(this.editYearInputs.harvest, item.year, item.harvest);
                    this.$set(this.editYearInputs.energyScale, item.year, item.energyScale);
                });
            },
            //获取月的input的值
            getMonthInputs() {
                return JSON.parse(JSON.stringify(this.editMonthInputs))
            },
            //设置月的input的值
            setMonthInputs(data) {
                this.clearObj(this.editMonthInputs.harvest);
                this.clearObj(this.editMonthInputs.energyScale);
                Array.from(data).forEach(item => {
                    this.$set(this.editMonthInputs.harvest, item.month, item.harvest);
                    this.$set(this.editMonthInputs.harvest, item.month + 'id', item.id);
                    this.$set(this.editMonthInputs.energyScale, item.month, item.energyScale);
                });
            },
            //清空数据
            clearObj(obj) {
        ...
        mounted() {
            ...
            this.activeYear = this.year;
            let me = this;
            let upLimit = function () {
                if (me.indexK1 > 5) {
                    me.indexK1 = 0
                } else if (me.indexK1 < 0) {
                    me.indexK1 = 5
                }
            }, uplimit2 = () => {
                if (me.yearIndex1 > 2) {
            ...
            let leftLimit = function () {
                if (me.indexK2 > 6) {
                    me.indexK2 = 0
                } else if (me.indexK2 < 0) {
                    me.indexK2 = 6
                }
            }, leftLimit2 = () => {
                if (me.yearIndex2 > 6) {
                    me.yearIndex2 = 0
                } else if (me.yearIndex2 < 0) {
                    me.yearIndex2 = 6
                }
            }
            this.$refs.baseCleandar.addEventListener('keydown', function (e) {
                switch (e.keyCode) {
                    //上键
                    case 38:
                        me.days[me.indexK1][me.indexK2].selected = false
                        me.indexK1--;
                        upLimit();
                        me.$refs['calendarInput' + me.indexK1 + me.indexK2][0].focus()
                        me.days[me.indexK1][me.indexK2].selected = true
                        break;
                    //右键
                    case 39:
                        me.days[me.indexK1][me.indexK2].selected = false
            ...
            this.$refs.baseCalendarYears.addEventListener('keydown', function (e) {
                switch (e.keyCode) {
                    //上键
                    case 38:
//                        me.days[me.indexK1][me.indexK2].selected = false
                        if (!me.yearIndex1) {
                            me.activeYear -= 7;
//                            me.$refs['calendarYearInput' + me.activeYear].focus()
                        }
                        me.yearIndex1--;

                        uplimit2();
//                        let index=me.activeYear-
//                        me.$refs['calendarYearInput' +  me.activeYear].focus()
//                        me.days[me.indexK1][me.indexK2].selected = true
                        break;
                    //右键
...
//                        me.days[me.indexK1][me.indexK2].selected = false
                        ...
                        leftLimit2();
//                        me.days[me.indexK1][me.indexK2].selected = true
                        me.$refs['calendarYearInput' + me.indexK1 + me.indexK2][0].focus()
                        break;
                    //下键
                    case 40:
//                        me.days[me.indexK1][me.indexK2].selected = false
...
//                        me.days[me.indexK1][me.indexK2].selected = true
        ...
        created() {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (let j = 0; j < 7; j++) {
                    this.editInputs[i].push({content: ""})
                }
            }
            ;
            this.yearIndex1 = 1;//行索引
            this.yearIndex2 = 3;///列索引
        },
        directives: {
            filterS(el, binding) {
                if (isNaN(parseFloat(binding.value))) {
                    ...
                    let point = parseFloat(binding.value).toString().split('.');
                    if (point[1] && point[1].length > 3) {
                        el.value = parseFloat(binding.value).toFixed(3)
                    }
                }
            }
        }
...
<style scoped>
    ...
    .calendar-prev.el-icon-d-arrow-left, .el-icon-d-arrow-right.calendar-next {
        font-size: 14px;
        width: 30px;
        position: relative;
        top: 11px;
    ...
    .calendar-years input, .calendar td input {
        width: calc(100% - 4px);
    }
    ...
    /*color: #999;*/
    /*}*/
    .calendar-months > label, .calendar-years > label {
        margin: 1px 5px;
        display: inline-block;
        width: 60px;
        line-height: 30px;
    ...
    .calendar-years > label, .calendar-months > label {
        width: 150px;
    }

    .calendar-years > label > label, .calendar-months > label > label {
        display: block;
    }

    .calendar-years > label > label:first-of-type, .calendar-months > label > label:first-of-type {
        text-indent: 28px;
    }

    .calendar-years > label > span, .calendar-months > label > span {
        position: relative;
        left: 27px;
        display: inline-block;
        width: 50px;
    ...
    .calendar-months span.active, .calendar-years span.active {
    ...
    .calendar-years input, .calendar-months input {
        width: 70px;

        /*box-shadow: 0px 0px 0px #aaa inset;*/
        outline: 0;
        /*box-shadow: rgba(255, 255, 255, 0.1) 0px 0px 8px inset;*/
        padding: 4px;
